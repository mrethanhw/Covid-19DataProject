...
import matplotlib.pyplot as plt
import pandas as pd
newdeaths_df = pd.read_excel('./Assembled Data/USA_States_COVID-19-NewDeathPer100k_2020-10-11.xlsx', index_col = 'States', parse_dates=True)
newdeaths_rolling_df = ((newdeaths_df.rolling(7, axis=1).sum())/7).fillna(0)
newcases_df = pd.read_excel('./Assembled Data/USA_States_COVID-19-NewPositivePer100k_2020-10-10.xlsx', index_col = 'States', parse_dates=True)
newcases_rolling_df = ((newcases_df.rolling(7, axis=1).sum())/7).fillna(0)
...
fig, ax = plt.subplots(nrows = 5, ncols = 2, figsize = (40, 20))
for clusternum in range(1, 5):
    c_df = cluster_df[cluster_df['Cluster']== clusternum]
    print("Cluster ", clusternum)
    cluster_newcases = newcases_rolling_df.loc[c_df.State.tolist()].fillna(0)
...
fig2, ax2 = plt.subplots(nrows = 5, ncols = 2, figsize = (40, 20))
for clusternum in range(1, 5):
    c_df = cluster_df[cluster_df['Cluster']== clusternum]
    cluster_newdeaths = newdeaths_rolling_df.loc[c_df.State.tolist()].fillna(0)
    z_scores = scipy.stats.zscore(cluster_newdeaths, nan_policy='omit')
    abs_z_scores = np.nan_to_num(np.abs(z_scores))
    filtered_entries = (abs_z_scores < 3).all(axis = 1)
    new_df = cluster_newdeaths[filtered_entries]
    new_df.describe().loc['mean'].plot(ax= ax2[(clusternum - 1)][0])
    cluster_newdeaths.describe().loc['mean'].plot(ax=ax2[(clusternum - 1)][1])
fig3, ax3 = plt.subplots(nrows = 2, ncols = 1, figsize = (20, 15))
ax3[0].set_title('Average Daily COVID-19 Cases per 100,000 people (Rolling 7-Day Average) with Outliers Omitted')
ax3[1].set_title('Average Daily COVID-19 Cases per 100,000 people (Rolling 7-Day Average) By Cluster')
ax3[0].set_xlabel('Date')
ax3[1].set_xlabel('Date')
ax3[0].set_ylabel('Average COVID-19 Cases per 100,000 People')
ax3[1].set_ylabel('Average COVID-19 Cases per 100,000 People')
...
for clusternum in range(1, 5):
    ...
    filtered_entries = (abs_z_scores < 3).all(axis = 1)
    new_df = cluster_newcases[filtered_entries]
    new_df.describe().loc['mean'].rename('Cluster ' + str(clusternum)).plot(ax= ax3[0], legend=True)
    cluster_newcases.describe().loc['mean'].rename('Cluster ' + str(clusternum)).plot(ax=ax3[1], legend=True)
